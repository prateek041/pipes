...
// GenericMapStage implements the GenericStage interface for the Map primitive.
type GenericMapStage[T any] struct {
	config   Config
	userFunc func(T) T
}

// NewGenericMapStage creates a new GenericMapStage.
func NewGenericMapStage[T any](cfg Config, fn func(T) T) *GenericMapStage[T] {
	return &GenericMapStage[T]{
		config:   cfg,
		userFunc: fn,
	}
}
...
func (s *GenericMapStage[T]) Name() string {
	return "GenericMap"
}

// ProcessBatch applies the user's Map function to every event in the batch.
func (s *GenericMapStage[T]) ProcessBatch(batch []T) ([]T, error) {
...
func (s *GenericMapStage[T]) Connect(wg *sync.WaitGroup, inChan <-chan T, outChan chan<- T, emitter Emitter) error {
	defer wg.Done()
